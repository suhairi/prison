@extends('layouts.app')

@section('content')

<?php use Carbon\Carbon; ?>

<div class="row">
	@include('layouts.messages')
	<div class="row">
		<h4>Modify Order</h4>
	</div>
	<hr />

	<div class="row">

		<table class="table table-bordered">
			<tr>
				<td width="25%"><strong>Name </strong></td>
				<td>{{ $order->users->name }}</td>
			</tr>
			<tr>
				<td><strong>No KP </strong> </td>
				<td>{{ $order->users->username }}</td>
			</tr>
			<tr>
				<td><strong>No SMPP </strong> </td>
				<td>{{ $order->users->nosmpp }}</td>
			</tr>
			<tr>
				<td><strong>Status </strong></td>
				<td>{{ strtoupper($order->users->status) }}</td>
			</tr>
			<tr>
				<td><strong>Date </strong></td>
				<td>{{ $order->created_at }}</td>
			</tr>
		</table>

		<form method="POST" action="{{ route('user.modifyOrder') }}"> 
			@csrf
			<input type="hidden" name="id" value="{{ $order->id }}">
			<table class="table table-bordered table-striped table-hover" id="tblProducts">
				<thead>
					<th>Bil</th>
					<th>Products</th>
					<th>Price (RM)</th>
					<th>Quantity</th>
					<th>Subtotal</th>
				</thead>
				<?php $grandTotal = 0; ?>
				@foreach($products as $product)

					@if($order->products->contains('id', $product->id))
						<?php 
							$prod = $order->products->where('id', $product->id)->first();
							$quantity = $prod->pivot->quantity;
							$subtotal = $product->price * $quantity;
							$grandTotal += $subtotal;
						?>
					@else
						<?php
							$quantity = 0;
							$subtotal = "0.00"
						?>
					@endif
					<tbody>
						<tr>
							<td valign="middle" align="center">{{ $loop->iteration }}</td>
							<td valign="middle">{{ $product->name }}</td>
							<td align="right" valign="middle">
								<input class="form-control form-control-sm text-align-right price" id="price" value="{{ number_format($product->price, 2) }}" name="price[{{ $product->id }}]" readonly>
							</td>
							<td width="10%">
								<input id="{{ $product->id }}" class="form-control form-control-sm w-10 qty" id="qty" type="number" step=1 name="qty[{{ $product->id }}]" value="{{ $quantity }}" required placeholder="Quantity" onClick="this.select();" />
							</td>
							<td align="right" valign="middle" width="10%">
								<input class="form-control form-control-sm text-align-right subtot" value="{{ number_format($subtotal, 2) }}" name="subtot" readonly>
							</td>
						</tr>
					</tbody>
				@endforeach
				<tfoot>
					<tr>
						<td colspan="4" align="right"><strong>JUMLAH BESAR (RM)</strong></td>
						<td align="right"><input name="grandTotal" class="grdtot" value="{{ number_format($grandTotal, 2) }}" readonly></td>
					</tr>
				</tfoot>
			</table>
			<div align="right">
				<button type="submit" class="btn btn-primary btn-sm" id="btnSubmit">
	                <span data-feather="user-plus"></span> 
	                Modify Order
	            </button>
            </div>

		</form>
	</div>

</div>


@endsection